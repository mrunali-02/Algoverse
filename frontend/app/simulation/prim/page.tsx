...
import { generatePrimSteps } from "@/utils/algorithms/graph/steps";
...
export default function PrimSimulationPage() {
  ...
  useEffect(() => {
    if (nodes.length > 0) {
      ...
      const generatedSteps = generatePrimSteps(nodes, edges, startId, false);
  ...
  useEffect(() => {
    if (currentStep?.action === "FINISHED") {
      progressService.recordProgress("prim", "COMPLETED", 100);
    }
  }, [currentStep?.action]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-[1700px] w-full mx-auto px-4 py-6">
        <VisualizerHeaderActions algorithmTitle="Prim's MST Algorithm" algorithmId="prim" />
