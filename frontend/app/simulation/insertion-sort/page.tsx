...
import { generateInsertionSortSteps } from "@/utils/algorithms/sorting/steps";
import { progressService } from "@/services/progressService";

export default function InsertionSortPage() {
  const [array, setArray] = useState<number[]>([43, 21, 56, 12, 89, 32]);

  const { steps, currentStepIndex, setSteps } = useSimulationStore();

  useEffect(() => {
    const generatedSteps = generateInsertionSortSteps(array);
  ...
  useEffect(() => {
    if (currentStep?.action === "FINISHED") {
      progressService.recordProgress("insertion-sort", "COMPLETED", 100);
    }
  }, [currentStep?.action]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-[1700px] w-full mx-auto px-4 py-6">
        <VisualizerHeaderActions algorithmTitle="Insertion Sort" algorithmId="insertion-sort" />

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-3">
            <TheoryPanel algorithmId="insertion-sort" />
          </div>
          ...
          <div className="lg:col-span-3 space-y-4 max-h-[820px] overflow-y-auto pr-1">
            ...
            <PseudocodePanel algorithmId="insertion-sort" activeLine={currentStep?.highlightedPseudocodeLine} />
          </div>
        </div>
      </main>
    </div>
