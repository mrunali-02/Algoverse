...
import { generateBFSSteps } from "@/utils/bfsEngine";
...
export default function BFSSimulationPage() {
  ...
  useEffect(() => {
    if (nodes.length > 0) {
      ...
      const generatedSteps = generateBFSSteps(nodes, edges, startId, isDirected);
  ...
  useEffect(() => {
    if (currentStep?.action === "FINISHED") {
      progressService.recordProgress("bfs", "COMPLETED", 100);
    }
  }, [currentStep?.action]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-[1700px] w-full mx-auto px-4 py-6">
        <VisualizerHeaderActions algorithmTitle="Breadth-First Search (BFS)" algorithmId="bfs" />
