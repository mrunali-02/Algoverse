...
import { generateBubbleSortSteps } from "@/utils/algorithms/sorting/steps";
...
export default function BubbleSortPage() {
  ...
  useEffect(() => {
    if (currentStep?.action === "FINISHED") {
      progressService.recordProgress("bubble-sort", "COMPLETED", 100);
    }
  }, [currentStep?.action]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-[1700px] w-full mx-auto px-4 py-6">
        <VisualizerHeaderActions algorithmTitle="Bubble Sort" algorithmId="bubble-sort" />

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-3">
            <TheoryPanel algorithmId="bubble-sort" />
          </div>
          ...
          <div className="lg:col-span-3 space-y-4 max-h-[820px] overflow-y-auto pr-1">
            ...
            <PseudocodePanel algorithmId="bubble-sort" activeLine={currentStep?.highlightedPseudocodeLine} />
          </div>
        </div>
      </main>
    </div>
