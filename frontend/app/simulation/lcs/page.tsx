...
import { generateLCSSteps } from "@/utils/algorithms/dynamicProgramming/steps";
import { progressService } from "@/services/progressService";

export default function LCSPage() {
  const [str1] = useState<string>("AGGTAB");
  const [str2] = useState<string>("GXTXAYB");

  const { steps, currentStepIndex, setSteps } = useSimulationStore();

  useEffect(() => {
    const generatedSteps = generateLCSSteps(str1, str2);
  ...
  useEffect(() => {
    if (currentStep?.action === "FINISHED") {
      progressService.recordProgress("lcs", "COMPLETED", 100);
    }
  }, [currentStep?.action]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-[1700px] w-full mx-auto px-4 py-6">
        <VisualizerHeaderActions algorithmTitle="Longest Common Subsequence (LCS)" algorithmId="lcs" />
